...
const BlogsDetails = () => {
  ...
  useEffect(() => {
    dispatch(blogSingleGetUser({ id: id }));
  }, []);
  ...
  const createBlogSchema = (data) => {
    const baseUrl = "https://www.starmebooks.com";
  
    const cleanBody = data?.content
      ?.replace(/&lt;/g, "<")
      ?.replace(/&gt;/g, ">")
      ?.replace(/<[^>]+>/g, "");
    ...
    return {
      ...
      "@graph": [
        ...
        {
          "@type": "WebPage",
          "@id": `${baseUrl}/blog/${data?._id}#webpage`,
          url: `${baseUrl}/blog/${data?._id}`,
          name: data?.title,
        ...
        {
          "@type": "BlogPosting",
          "@id": `${baseUrl}/blog/${data?._id}#blogposting`,
          headline: data?.title,
          name: data?.title,
          description: data?.description,
          ...
          author: {
            "@type": "Person",
            name: data?.authorName || "StarMe Team",
          },
          publisher: {
        ...
        {
          "@type": "BreadcrumbList",
          "@id": `${baseUrl}/blog/${data?._id}#breadcrumb`,
          itemListElement: [
            {
              "@type": "ListItem",
              position: 1,
        ...
        {
          "@type": "WebPage",
          "@id": `${baseUrl}/blog/${data?._id}#speakable`,
          speakable: {
...
<GeoHelmet
  title={data?.title}
  description={data?.description}
  keywords={`${data?.title}, blog, stories, kids stories, StarMe`}
  type="article"
  schema={createBlogSchema(data)}
/>
          ...
          <div className={` ${isNavbarOpen ? "blur-sm" : ""}  `}>
            <div className="max-w-7xl mx-auto  pt-6 font-figTree">
                  ...
                  <div className="max-w-3xl w-full h-full flex flex-col items-center">
                    ...
                    <div className="mb-1 w-full">
                      ...
                      <div className="block sm:hidden">
                        <div
                          className="mb-2 text-base font-bold font-figTree   cursor-pointer flex items-center justify-center gap-2 bg-white border-2 border-purple-500 text-purple-500 p-3 rounded-full transition-colors h-[40px]"
                          ...
                          <div className="py-4 space-y-2  px-[15px]   ">
                            {headers?.map((h, i) => {
                              return (
                                <div
                                  key={h.id}
                                  className={`cursor-pointer hover:text-purple-500 text-purple-400 text-[15px]  font-figTree transition-colors 
                                 ${
                                   h?.tag === "h2"
                                     ? "font-bold !mt-4"
                                     : h?.tag === "h3"
                    ...
                    {/* <div className="w-full p-[30px] [&_h2]:text-purple-500 [&_h2]:text-[28px] mb-[50px]  [&_h2]:mb-2 [&_h2]:font-semibold rounded-[28px] text-[#333]  [&_h3]:text-black [&_h3]:mt-1  [&_h3]:text-[20px]  [&_p]:mb-3 [&_p]:text-[17px] [&_a]:!text-purple-500   bg-white"> */}
                    <div
                      className="w-full px-[15px] py-[20px] sm:p-[30px] [&_h2]:text-purple-500 [&_h2]:font-normal [&_h2]:text-[25px] [&_h2]:sm:text-[28px]  mb-[20px] [&_h2]:mb-2
                 rounded-[28px] text-[#333] [&_h3]:text-black [&_h3]:mt-5 [&_h3]:text-[20px] [&_p]:mb-3 [&_p]:text-[17px]
               [&_a]:!text-purple-500   [&_strong]:!font-[800] bg-white"
                    >
                      <div
                        className=""
                        ...
                        <div className="space-y-1">
                          {headers?.map((h, i) => {
                            return (
                              <div
                                key={h.id}
                                className={`cursor-pointer hover:text-purple-500 text-[#333] text-[15px] font-figTree transition-colors          
                             ${
                               h?.tag === "h2"
                                 ? "font-normal !mt-4"
                                 : h?.tag === "h3"
              ...
              <div className=" pb-[50px] px-3">
                <div
                  className="text-purple-500   font-semibold text-[26px] sm:text-[34px]
