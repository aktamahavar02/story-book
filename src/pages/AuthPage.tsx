    ...
    <>
      ...
      <div className={` ${isNavbarOpen ? "blur-sm" : ''}  `}>
        <div className={`${isForgot ? "min-h-[calc(100vh-97px)] max-[425px]:items-center " : ""} flex justify-center  px-0 sm:px-16 xl:px-0 py-4 xl:py-0 relative`}>
          ...
          {/* 👇 main auth container */}
          <div
            className={` overflow-hidden flex flex-col md:flex-row ${extraClass}  w-full mx-auto gap-2 sm:gap-2 px-4 py-0 ${isReset ? " mt-0 sm:mt-[7.4%] h-fit" : isForgot ? " mt-0 sm:mt-[7.5%] h-fit" : "  md:p-12 md:pt-24"
              } mt-0`}
          >

            {/* left section */}
            <div
              className={` lg:${isForgot ? "w-full" : "w-1/2"}  w-full ${isForgot ? " md:w-full" : "md:w-1/2"
